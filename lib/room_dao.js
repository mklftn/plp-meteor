Meteor.methods({
  ...
  deleteRoom: function (idSchool, roomName) {
    // Make sure the user is logged in before inserting a task
    if (! Meteor.userId()) {
      throw new Meteor.Error("not-authorized");
    }
    Schools.update({"_id" : idSchool}, { "$pull": { "rooms" : {"nom" : roomName}}} );
  ...
  updateRoomName: function (idSchool, oldRoomName, newRoomName) {
    // Make sure the user is logged in before inserting a task
    if (! Meteor.userId()) {
      throw new Meteor.Error("not-authorized");
    }
    Schools.update({"_id": idSchool, "rooms.nom": oldRoomName },{ "$set" : { "rooms.$.nom" : newRoomName } });
  },
